...
This package is basically forked from jenssegers/laravel-mongodb and adds functionalities to the Eloquent model and Query builder for MongoDB, using the original Laravel API. *This library extends the original Laravel classes, so it uses exactly the same methods.*
...
```bash
$ composer require softuvo/mongodb
```
